...
export default function StepPin() {
  ...
  async function login() {
    const { data } = await supabase
      .from('kasir').select('*')
      .eq('id', kasir.id).eq('pin', pin).single()
  ...
  return (
    <div>
      ...
      <input
        ...
        id="pin-input"
      />
      <label htmlFor="pin-input" className="block text-center text-sm text-blue-600 cursor-pointer mb-6 underline">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={login}
          className="flex-1 bg-blue-600 text-white py-3 rounded-lg flex items-center justify-center gap-2 font-medium hover:bg-blue-700 transition"
        >
          <Lock size={16} /> Masuk
        </button>
      </div>
